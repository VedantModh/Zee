...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mobileNavLinks = document.querySelectorAll('.mobile-nav-list a, .mobile-menu-button .btn-gradient');
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const finish = () => {
        ...
        if (window.gsap) {
            ...
            const logo = active ? active.querySelector('.home-logo') : document.querySelector('.home-logo');
...
function initScrollAnimations() {
    ...
    setTimeout(() => {
        ...
        if (gradientLine) gsap.set(gradientLine, { scaleX: 0, transformOrigin: 'center' });
        ...
        if (whoSection) {
            ...
            if (whoGradientLine) gsap.set(whoGradientLine, { scaleX: 0, transformOrigin: 'center' });
        ...
        if (recapSection) {
            const recapItems = recapSection.querySelectorAll('.recap-item:not(.recap-hidden)');
        ...
        if (registerSection) {
            ...
            const registerGradientLine = registerSection.querySelector('.about-gradient-line');
            ...
            if (registerGradientLine) gsap.set(registerGradientLine, { scaleX: 0, transformOrigin: 'center' });
        ...
        if (agendaSection) {
            ...
            const agendaGradientLine = agendaSection.querySelector('.about-gradient-line');
            ...
            if (agendaGradientLine) gsap.set(agendaGradientLine, { scaleX: 0, transformOrigin: 'center' });
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (registerForm) {
        ...
        const mobileInput = registerForm.querySelector('#mobile');
        
        // Special handling for mobile input - add +91 on focus if empty
        if (mobileInput) {
            mobileInput.addEventListener('focus', () => {
                // If field is empty, add +91
                if (mobileInput.value.trim() === '' || mobileInput.value === '+91 ') {
            ...
            mobileInput.addEventListener('keydown', (e) => {
                ...
                // If trying to delete +91 prefix (including space), prevent it
                if (cursorPos <= 4 && (e.key === 'Backspace' || e.key === 'Delete')) {
                    if (mobileInput.value.startsWith('+91 ')) {
                        e.preventDefault();
                        mobileInput.setSelectionRange(4, 4);
                    }
                }
            });
            
            // Ensure +91 stays at the beginning and handle input
            mobileInput.addEventListener('input', (e) => {
                let value = mobileInput.value;
                
                // If user deleted +91 or it's not at start, fix it
                if (!value.startsWith('+91 ')) {
                    ...
                    // Place cursor at end
                    setTimeout(() => {
                        mobileInput.setSelectionRange(value.length, value.length);
                    }, 0);
                }
                
                // Limit to reasonable length (country code + space + 10 digits = 14)
                if (value.length > 14) {
                    mobileInput.value = value.substring(0, 14);
                }
            });
        }
        ...
        formFields.forEach(input => {
            ...
            input.addEventListener('input', () => {
                // For mobile input, check if there's more than just +91
                if (input === mobileInput) {
                    const trimmedValue = input.value.trim();
                    const hasValue = trimmedValue !== '' && trimmedValue !== '+91' && trimmedValue !== '+91 ' && input.value.length > 4;
                    if (hasValue) {
            ...
            input.addEventListener('blur', () => {
                ...
                if (input === mobileInput) {
                    ...
                    if (!hasValue) {
                        // If only +91 or empty, clear it on blur
                        if (trimmedValue === '' || trimmedValue === '+91' || trimmedValue === '+91 ') {
                            input.value = '';
                        }
                        // Remove filled class
                        formField.classList.remove('filled');
...
const cityData = {
    pune: {
        ...
        timeline: [
            { time: '05:00 PM - 5:30 PM', title: 'Welcome & Registration + Hi-tea', desc: 'Guest arrivals and registration', side: 'right' },
            { time: '05:30 PM - 6:00 PM', title: 'Opening Address: Ms. Laxmi Shetty', desc: 'Welcome note and an overview of Z\'s Omnichannel Ecosystem', side: 'left' },
            { time: '6:00 PM - 6:30 PM', title: 'Speaker Session 2: Mrs. Zelam Chaubal', desc: '', side: 'right' },
            { time: '6:30 PM - 7:00 PM', title: 'Speaker Session 1: Mr. Tanmay Kanitkar', desc: '', side: 'left' },
            { time: '7:00 PM - 7:30 PM', title: 'Speaker Session 3: Mr. Girish Chitale', desc: '', side: 'right' },
            { time: '7:30 PM - 7:45 PM', title: 'Speaker Session 4: Mr. Manoj Gokhale', desc: 'Media&TA', side: 'left' },
            { time: '7:45 PM - 8:15 PM', title: 'VC Session 5: Mr. Sunil Musti', desc: '', side: 'right' },
            { time: '8:15 PM - 8:45 PM', title: 'Z Network Ecosystem Showcase: Ms. Laxmi Shetty + Mr. Gunjarav Nayak', desc: 'Showcase of \'Z\' Channels omnichannel strength + custom offering packages', side: 'left' },
            { time: '8:45 PM Onwards', title: 'Closing Note + Networking Cocktails + Dinner', desc: '', side: 'right' },
        ],
        speakers: [
            { name: ' Mr. Manoj Gokhale ', title: 'Director', company: 'Media&TA', image: './Zee All Images:Videos/Speakers cm7.jpg' },
    ...
    kolkata: {
        ...
        timeline: [
            ...
            { time: '10:20 AM – 10:40 AM', title: 'Speaker Session 1: Mr. Abhijit Roy', desc: '"INNOVATION- Core engine of brand growth"', side: 'right' },
            { time: '10:40 AM – 11:00 AM', title: 'Speaker Session 2: Ms. Poulomi Roy', desc: 'Scaling with Story: The Strategic Role of Media in Brand Growth', side: 'left' },
            { time: '11:00 AM - 11:20 AM', title: 'Speaker Session 4: Mr Ashish Beriwala', desc: 'Success of SRMB and its journey and the rise of MSME', side: 'right' },
            { time: '11:50 AM - 12:20 PM', title: 'Fireside Chat: Moderated by Mr Samrat Sengupta', desc: 'Mr Samrat Sengupta - AVP, Brand Building, Ultratech Cement | Cultural Capital to Market Capital: Building Scalable MSMEs from the East | Panelists: • Mr. Ankit Daga - Director, Mcnroe Consumer Products Pvt. Ltd. • Ms. Poulami Roy - CMO, Joy Personal Care (RSH Global) • Mr. Muralikrishnan L - CMO & Co-founder, Wow Momo • Dr. Suborno Bose - Chairman & CEO (I.I.H.M) & Indismart Group Worldwide', side: 'left' },
            { time: '12:20 PM – 12:40 PM', title: 'Speaker Session 4: Ms. Swarnali Kanjilal', desc: 'Changing Customer Preference', side: 'right' },
            { time: '12:40 PM – 01:00 PM', title: 'Speaker Session 5: Mr. Mahesh Motwani & Mr. Debarshi Chakravorti', desc: 'The Media Multiplier: Media Impact on Brands in Indian', side: 'left' },
            { time: '01:00 PM Onwards', title: 'Closing Note + Breakout Session', desc: '', side: 'right' },
            { time: '02:00 PM Onwards', title: 'Networking Lunch', desc: '', side: 'left' }
        ],
        speakers: [
            // { name: 'Mr.Ashish Sehgal', title: 'Chief Growth Officer', company: 'Zee Entertainment Enterprises Ltd.', image: './Zee All Images:Videos/Speakers 1.png' },
            ...
            { name: 'Mr. Mahesh Motwani', title: 'Executive VP', company: 'Initiative Media', image: './Zee All Images:Videos/Speakers 7.png' },
            { name: 'Mr. Debarshi Chakravorti', title: 'VP, Business & Communications Planning', company: 'Interactive Avenues', image: './Zee All Images:Videos/Speakers 8.png' },
            { name: 'Mr. Samrat Sengupta', title: 'AVP, Brand Building', company: 'Ultratech Cement', image: './Zee All Images:Videos/Speakers 9.png' },
    ...
    chennai: {
        ...
        timeline: [
            { time: '9:00 AM - 10:00 AM', title: 'WELCOME & REGISTRATION + BREAKFAST', desc: 'Guest arrivals and registration', side: 'right' },
            { time: '10:00 AM - 10:20 AM', title: 'OPENING ADDRESS: MR.ASHISH SEHGAL', desc: 'Welcome note and an overview of Z\'s omnichannel Ecosystem', side: 'left' },
            { time: '10:20 AM – 10:40 AM', title: 'Speaker Session 1: Shri. BA. Ramesh', desc: 'Tradition Meets Transformation: The Thangamayil Way of Building a Future-Ready Retail Brand', side: 'right' },
            { time: '10:40 AM – 11:00 AM', title: 'Speaker Session 2: Mr. Senthil Kumar Suthanandam', desc: 'Churn, Don’t Burn: Sustaining Legacy Brands in the Age of Disruption', side: 'left' },
            { time: '11:00 AM - 11:30 AM', title: 'Z Network Ecosystem Showcase: Ms. Laxmi Shetty & Mr. Rajnish Gupta', desc: 'Showcase of \'Z\' Channels omnichannel strength & custom offering packages', side: 'right' },
            { time: '11:30 AM - 11:50 AM', title: 'Speaker Session 3: Mr. Navaraja Murugan', desc: 'From Local Legacy to Lifestyle Leader: Navigating Ecommerce with Purpose & People', side: 'left' },
            { time: '11:50 AM – 12:10 PM ', title: 'Speaker Session 4: Mr. Chandrasekar Kupperi', desc: 'CCapital Meets Culture: Backing Bharat’s Next-Gen Brands with Local Soul and Global Ambition', side: 'right' },
            { time: '12:10 PM - 12:40 PM', title: 'Fireside Chat: Moderated by Mr. K. Satyanarayana', desc: 'Mr. K. Satyanarayana - Sr. Vice President - R.K. Swamy Media Group | Panel Topic: Decoding the TN Playbook – Growth, Granularity & the Power of Regional Love', title: 'Panelists:', desc: '• Mr. Hari Hara Sudhan - Head of Marketing Communication - Nippon Paint India |• Mr. A. Senthil Kumar - Chief Business Officer - Thalappakatti Hotels Pvt. Ltd.|• Mr. Arvind Naidu - Head of Marketing - KTV Health Foods (Sunland) | • Mr. Ashish Sehgal - Chief Growth Officer - Zee Entertainment Enterprises Ltd.,• Mr. Arvind Naidu - Head of Marketing - KTV Health Foods (Sunland)• Mr Ashish Sehgal - Chief Growth Officer - Zee Entertainment Enterprises Ltd.', side: 'left' },
            { time: '12:40 PM Onwards', title: 'Closing Note + Breakout Session', desc: '', side: 'right' },
            { time: '01:00 PM Onwards', title: 'Networking Lunch', desc: '', side: 'left' }
        ], 
        speakers: [
            // { name: 'Mr.Ashish Sehgal', title: 'Chief Growth Officer', company: 'Zee Entertainment Enterprises Ltd.', image: './Zee All Images:Videos/Speakers 1.png' },
            { name: 'Mr. Arvind Naidu', title: 'Head of Marketing', company: 'KTV Health Foods (Sunland)', image: './Zee All Images:Videos/Speakers 2.png' },
            { name: 'Mr. A. Senthil Kumar', title: 'Chief Business Officer', company: 'Thalappakatti Hotels Pvt. Ltd.', image: './Zee All Images:Videos/Speakers c3.png' },
            { name: 'Mr. Navaraja Murugan', title: 'Director', company: 'Darling Digital World Pvt Ltd.', image: './Zee All Images:Videos/Speakers c4.png' },
            { name: 'Mr. K. Satyanarayana', title: 'Sr. Vice President', company: 'R.K. Swamy Media Group', image: './Zee All Images:Videos/Speakers c5.png' },
            { name: 'Mr. Hari Hara Sudhan', title: 'Head of Marketing Communication', company: 'Nippon Paint India', image: './Zee All Images:Videos/Speakers c6.png' },
    ...
    bangalore: {
        ...
        timeline: [
            ...
            { time: '10:45 AM – 11:05 AM', title: 'Speaker Session 1 - Mrs. Rasika Prashant', desc: '"Winning attention in cluttered world - FMCG strategies in influencing modern consumer"', side: 'right' },
            { time: '11:05 AM – 11:25 AM', title: 'Speaker Session 2: Mr. Shuvadip Banerjee', desc: '"From shelf to scrolls - Reimagining the FMCG growth with new age media and consumer insights"', side: 'left' },
            { time: '11:25 AM – 11:45 AM', title: 'Speaker Session 3: Mr. Rahul Joseph', desc: '"From 5 stores to INR 1,500 Crs, turning trust into real treasure - The White Gold Revolution"', side: 'right' },
            { time: '11:45 AM – 12:15 PM ', title: 'Z Network Ecosystem Showcase: Mrs. Laxmi + Mr. V.S. Raghavan', desc: 'Showcase of \'Z\' Channels omnichannel strength + custom offering packages', side: 'left' },
            { time: '12:15 PM – 12:35 PM', title: 'Speaker Session 4: Mr. Rajesh Dembla', desc: '"What makes VCs say YES - Decoding startup success from pitch to scale and media levers that drive it"', side: 'right' },
            { time: '12:35 PM – 12:55 PM', title: 'Speaker Session 5: Mr. Ramesh Arvind', desc: '', side: 'left' },
            { time: '12:55 PM - 01:30 PM', title: 'Fireside Chat: Moderated by Mrs. Aparna Thakur', desc: 'Aparna Thakur - Sr. VP, HiveMinds | "One Bharat many screens - Navigating omni-channel India with brand purpose" | Panelists:', desc: '• Ashish Sehgal - Chief Growth Officer, Zee Entertainment Enterprises Ltd |• Nischay AG - Co-founder & CEO, JAR |• Venkatesh Vijayaraghavan - MD and CEO, TTK prestige | • Shuvadip Banerjee - Chief Digital Marketing Officer, ITC Ltd', side: 'right' },
            { time: '01:30 PM Onwards', title: 'Closing Note + Breakout Session', desc: '', side: 'left' },
            { time: '02:00 PM Onwards', title: 'Networking Lunch', desc: '', side: 'right' }
        ],
        speakers: [
            // { name: 'Mr.Ashish Sehgal', title: 'Chief Growth Officer', company: 'Zee Entertainment Enterprises Ltd.', image: './Zee All Images:Videos/Speakers 1.png' },
            { name: 'Nischay AG', title: 'Co-founder & CEO', company: 'JAR', image: './Zee All Images:Videos/Speakers b2.png' },
            { name: 'Rahul Joseph', title: 'Founder', company: 'White Gold', image: './Zee All Images:Videos/Speakers b3.png' },
            { name: 'Rajesh Dembla', title: 'Growth Visionary & Innovation', company: 'Backer', image: './Zee All Images:Videos/Speakers b4.png' },
            { name: 'Ramesh Arvind', title: 'Actor and Film Maker', company: '', image: './Zee All Images:Videos/Speakers b5.png' },
    ...
    delhi: {
        ...
        timeline: [
            { time: '9:00 AM - 10:00 AM', title: 'WELCOME & REGISTRATION + BREAKFAST', desc: 'Guest arrivals and registration', side: 'right' },
            { time: '10:00 AM - 10:20 AM', title: 'OPENING ADDRESS: MR.ASHISH SEHGAL', desc: 'Welcome note and an overview of Z\'s omnichannel Ecosystem', side: 'left' },
            { time: '10:20 AM – 10:40 AM', title: 'Speaker Session 1: Kunal Sharma', desc: 'In-Grained: When Culture, Consumers and Commitment collide', side: 'right' },
            { time: '10:40 AM – 11:00 AM', title: 'Speaker Session 2: Saurabh Munjal', desc: 'Breaking the Bottle: How Regional Beverages Are Uncapping Global Giants', side: 'left' },
            { time: '11:00 AM - 11:20 AM', title: 'Speaker Session 3: Manik Pasricha', desc: 'Unlocking Investment Opportunities for Start-Ups', side: 'right' },
            { time: '11:20 AM - 11:40 AM', title: 'Speaker Session 4: Harkawal Singh', desc: 'Elevating Brand Performance: In a Cluttered Ecosystem', side: 'left' },
            { time: '11:40 AM – 12:10 PM ', title: 'Z Network Ecosystem Showcase: Laxmi + Ali', desc: 'Showcase of Zee Network\'s omnichannel strengths + Custom Offering Packages', side: 'right' },
            { time: '12:15 PM - 01:00 PM', title: 'Fireside Chat: Moderated by Mr. Mohit Joshi - Havas Media', desc: 'Mr. Mohit Joshi - CEO - Havas Media| Culture as Currency : Building Brands that belong in Bharat| Panelists:', desc: '• Ashish Sehgal - Chief Growth Officer, Zee Entertainment Enterprises Ltd| • Raj Vikram - Co-founder & CEO, Beast Life |• Harkanwal Singh - Vice President, Marketing - Rohit Surfactant Pvt Ltd.| • Saurabh Munjal - Co-Founder and Chief Executive Officer - Lahori Zeera|• Manav Sethi - Head Media Martech & Growth - VIDA', side: 'left' },
            { time: '01:00 PM Onwards', title: 'Closing Note + Breakout Session', desc: '-', side: 'right' },
            { time: '02:00 PM Onwards', title: 'Networking Lunch', desc: '-', side: 'left' }
        ],
        speakers: [
    ...
    mumbai: {
        ...
        timeline: [
            ...
            { time: '10:00 AM – 10:20 AM', title: 'Opening Address', desc: 'Ashish Sehgal - Chief Growth Officer, Zee Entertainment Enterprises Ltd. | Welcome note and an overview of Z’s Omnichannel Ecosystem; introduction to the 7-language content strategy', side: 'right' },
            { time: '10:20 AM – 10:35 AM', title: 'Time to RISE – Our Road to Effectiveness in Brand Connect', desc: 'Raghavendra Katte – AVP, Brand Performance, Jyothi Labs Ltd. | Marketing insights on brand performance and Z’s growth strategy', side: 'left' },
            { time: '10:35 AM – 10:50 AM', title: 'Legacy to Leadership – The Waman Hari Pethe Growth Story', desc: 'Ashish Pethe - Partner, Waman Hari Pethe Jewellers | Retail transformation through generations and building brand legacy', side: 'right' },
            { time: '10:50 AM – 11:05 AM', title: 'Regional Roots, National Shine – Pitambari’s Brand Transformation Story', desc: 'Parikshit Prabhudesai – Vice Chairman, Pitambari Products Pvt. Ltd. | From a regional player to a national brand – the Pitambari journey', side: 'left' },
            { time: '11:05 AM - 11:20 AM', title: 'Beyond Capital – How Storytelling, Strategy & Scale Drive VC-Backed Brands', desc: 'Parikshit Prabhudesai – Vice Chairman, Pitambari Products Pvt. Ltd. | Vansh Oberoi – Managing Partner, VC Grid', side: 'right' },
            { time: '11:20 AM – 11:35 PM', title: 'From Reels to Reality – The AI Powered Future of Content', desc: 'Shubhranshu Singh – Chief Marketing Officer, TATA Motors | AI-driven content strategies and evolving consumer media behavior', side: 'left' },
            { time: '11:35 PM – 11:50 PM', title: 'The Evolving Role of Television in Today’s Media Landscape', desc: 'Ashwin Padmanabhan – Chief Operating Officer, WPP Media South Asia | Fresh perspectives on the relevance and evolution of TV in a digital era', side: 'right' },
            { time: '11:50 PM - 12:10 PM', title: 'Short Break', desc: 'Refreshments and informal networking', side: 'left' },
            { time: '12:10 PM - 12:40 PM', title: 'Panel – Roots and Reach: Traditional Media’s Role in a Connected India', desc: 'Panellist’s: Raghavendra Katte, Ajay Gupta, Ashish Sehgal, Shubhranshu Singh, Ashish Pethe |Cross-industry discussion on media’s influence and evolving consumer habits', side: 'right' },
            { time: '12:40 PM – 1:10 PM', title: 'Z Network Ecosystem Showcase + Custom Offering Packages', desc: 'Laxmi Shetty - Chief Sales Strategy & Maximisation Officer, Gunjarav Nayak - Chief Sales Officer | Showcase of Zee Network’s omnichannel strengths and tailored advertising solutions', side: 'left' },
            { time: '1:10 PM – 1:20 PM', title: 'Closing Note', desc: 'Ashish Sehgal - Chief Growth Officer, Zee Entertainment Enterprises Ltd. | Wrap-up and closing remarks for the day | Wrap-up and closing remarks for the day', side: 'right' },
            { time: '1:20 PM – 2:00 PM', title: 'Breakout Zone – Informal Interactions & Networking', desc: 'Z Sales Cluster Heads | Open conversation zone for clients and Zee teams', side: 'left' },
            { time: '2:00 PM Onwards', title: 'Networking Lunch', desc: '- | Group lunch and casual networking', side: 'right' },
        ],
        speakers: [
            // { name: 'Mr.Ashish Sehgal', title: 'Chief Growth Officer', company: 'Zee Entertainment Enterprises Ltd.', image: './Zee All Images:Videos/Speakers 1.png' },
...
function updateSpeakers(speakersData) {
    const speakersWrapper = document.querySelector('.speakers-slider .swiper-wrapper');
